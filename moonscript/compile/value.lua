...
unpack = util.unpack
local append = insert
local is_checkparams
is_checkparams = function(node)
	return node and (node[1] == '~' or (node[1] == '~<' and node[#node] == '>'))
end
local table_delim = ","
local make_chain_call
make_chain_call = function(chain, params)
	local refv = chain[1]
	local cpt = {
		"chain",
		{
...
return {
	...
	fndef = function(self, node)
		...
		do
			...
			for _index_0 = 1, #args do
				...
				if (is_checkparams(default_value)) then
					table.remove(arg, 2)
					checkparams = default_value
					default_value = arg[2]
					table.insert(arg, #arg + 1, checkparams)
				end
				...
				if checkparams then
					insert(check_lines, self:check_type(name, checkparams))
				end
		...
		do
			...
			for _index_0 = 1, #check_lines do
				local clines = check_lines[_index_0]
				local typecheck, rangecheck
				typecheck, rangecheck = clines[1], clines[2]
				_with_0:stm(typecheck)
				for _index_1 = 1, #rangecheck do
					local tline = rangecheck[_index_1]
					_with_0:stm(tline)
				end
			end
			_with_0:stms(block)
	...
	opte = function(self, value)
		local v1, op, sign, v2
		do
			local _obj_0 = value[2]
			v1, op, sign, v2 = _obj_0[1], _obj_0[2], _obj_0[3], _obj_0[4]
		end
		local optransmap = {
			['+'] = 'append'
		}
		local realop = optransmap[op]
		local newstm = make_chain_call({
			'table',
			'clone'
		}, {
			make_chain_call({
		...
		local statement = self:value(newstm)
		return statement
	end,
	optt = function(self, value)
		local v1, sign, op, v2
		do
			local _obj_0 = value[2]
			v1, sign, op, v2 = _obj_0[1], _obj_0[2], _obj_0[3], _obj_0[4]
		end
		local optransmap = {
			['+'] = 'clone'
		}
	...
	optv = function(self, value)
		local v1, op, v2
		do
			local _obj_0 = value[2]
			v1, op, v2 = _obj_0[1], _obj_0[2], _obj_0[3]
		end
		local optransmap = {
			['+='] = 'addup',
			['-='] = 'subdown',
	...
	looparth2 = function(self, value)
		print("looparth")
		local p1, direction, inlineparamnum, p2
		do
			local _obj_0 = value[2]
			p1, direction, inlineparamnum, p2 = _obj_0[1], _obj_0[2], _obj_0[3], _obj_0[4]
		end
		local tb, f = p1, p2 or inlineparamnum
		local foreachtpl = {
			"decorated",
			{
				"chain",
