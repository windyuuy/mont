...
return {
	...
	assign = function(self, node)
		...
		local assign_line = self:line()
		do
			if #undeclared == #names and not has_fndef then
				assign_line:append(declare)
			else
				if #undeclared > 0 then
					self:add(declare, node[-1])
				end
				assign_line:append_list((function()
					local _accum_0 = { }
					local _len_0 = 1
					for _index_0 = 1, #names do
						local name = names[_index_0]
		...
		local newlines = self
		do
			newlines:add(assign_line)
		end
		local default_params, static_default_params
		for _index_0 = 1, #names do
			local _continue_0 = false
			repeat
				local avalue = names[_index_0]
				local params = avalue[3]
				if not (params) then
					_continue_0 = true
					break
				end
		...
		for _index_0 = 1, #names do
			...
			repeat
				local avalue = names[_index_0]
				local org_params = avalue[3]
				local sign = org_params and org_params[1]
				if (sign == '~~<') then
					default_params = org_params
				end
				local params = org_params or static_default_params or default_params
				if not (params) then
					_continue_0 = true
					break
				end
				local varname = avalue[2]
				local ret = self:check_type(varname, params)
				if not (ret) then
					_continue_0 = true
					break
				end
				local newline, typelines
				newline, typelines = ret[1], ret[2]
				if not (newline) then
					_continue_0 = true
					break
				end
				do
					newlines:stm(newline)
					for _index_1 = 1, #typelines do
						local tline = typelines[_index_1]
						newlines:stm(tline)
					end
				end
				_continue_0 = true
