...
is_checkparams=(node)->
	node and (node[1]=='~' or (node[1]=='~<' and node[#node]=='>'))
	
table_delim = ","

make_chain_call=(chain,params)->
	refv=chain[1]
	cpt={"chain",{"ref",refv,},}
	for i=2,#chain
		append(cpt,{'dot',chain[i]})
	append(cpt,{'call',params})
	cpt
...

{
	...
	fndef: (node) =>
		...
		arg_names = for arg in *args
			...
			if(is_checkparams default_value)
				table.remove(arg,2)
			...
			if checkparams
				insert check_lines, @check_type(name,checkparams)
			insert default_args, arg if default_value
			name

		-- print "check_lines"
		-- print util.dump check_lines
		...
		with @block!
			...
			for clines in *check_lines
				{typecheck,rangecheck}=clines
				\stm typecheck
				for tline in * rangecheck
					\stm tline
	...
	opte: (value)=>
		-- print "opte"
		-- print util.dump value
		{v1,op,sign,v2}=value[2]
		optransmap={'+':'append'}
		realop=optransmap[op]
		newstm=make_chain_call(
			{'table','clone'},
			{
				make_chain_call(
					{'table',realop},
		...
		statement=@value(newstm)
		statement
		
	optt: (value)=>
		-- print "optt"
		-- print util.dump value
		{v1,sign,op,v2}=value[2]
		optransmap={'+':'clone'}
		realop=optransmap[op]
		newstm=make_chain_call(
			{'table','clone'},
	...
	looparth2: (value)=>
		print "looparth"
		-- print util.dump value
		{p1,direction,inlineparamnum,p2}=value[2]
		-- print util.dump p2
		tb,f=p1,p2 or inlineparamnum
		-- tbn,fn=(concat @value tb),(concat @value f)
		-- statement=fn.."(v,i) for v,i in *"..tbn
		foreachtpl={
			"decorated",
			{
				"chain",
