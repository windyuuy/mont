...

{
	...
	assign: (node) =>
		...
		for avalue in *names do
			org_params=avalue[3]
			sign=org_params and org_params[1]
			if(sign=='~~<')
				default_params=org_params 
			params=org_params or static_default_params or default_params
			continue unless params

			varname=avalue[2]
			ret=@check_type(varname,params)
			continue unless ret
			{newline,typelines}=ret
			
			continue unless newline
			
			with newlines
				\stm newline
				for tline in *typelines
					\stm tline
					-- \stm {'assign', {'hello'}, {'323'}}
					-- \add @line tline
			
		return if newlines==self
		return newlines
		-- return if checkedonce
