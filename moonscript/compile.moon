...
conbine_callfunc=(fname,params)->
	(CodeGen{:fname,:params,tpl:"${fname}(${params; separator=', '})"})'tpl'
repr=(str)->
	'\''..str..'\''
...
class Block
	...
	call_fn:(varname,params)=>
		print "call_fn"
		print varname
		print util.dump params
	...
	check_type:(varname,params)=>
		-- print "lkjwelkjwlekjflkj"
		-- print varname
		-- print util.dump params
		
		make_range=(varname,lines,rangeparams)->
			-- tparams={repr(lines[#lines]),concat @value({"explist", unpack rangeparams}) }
			-- rangelines[#rangelines+1]=conbine_callfunc 'checkrange',tparams
			for subrange in * rangeparams do table.remove subrange[1],-1
			stt={{"ref",varname},lines[#lines], {"table",rangeparams}}
			@call_fn 'checkrange',stt
		...
		head=params[1]
			-- when '~'
		if(head=='~' or head=='~~')
			lines[#lines+1]=params[2]
			checktype=params[3]
			if(checktype)
				rangeparams=checktype[2]
				rangelines[#rangelines+1]=make_range varname,lines,rangeparams
		elseif head=='~<' or head=='~~<' or head==':~<'
			paramscount=0
			for checktype in * params[2,]
				if checktype==','
					paramscount=paramscount+1
					continue
				break if checktype=='>'
				if(type(checktype)=='table' and checktype[1]=='table')then
					if(paramscount>#lines)
						lines[#lines+1]={'string',"'",'number'}
						paramscount=#lines
					rangeparams=checktype[2]
					rangelines[#rangelines+1]=make_range varname,lines,rangeparams
				else
					-- insert checktype,2,"'"
					lines[#lines+1]=checktype
		else
			return nil
		...
		if(#lines>1)
			...
			fname='gontype'
		-- newline=concat{'gontype(',varname,",'",concat(lines,"','"),"')"}
		newline={"chain", {"ref", 'gontypes',},{"call",{
			{'ref',varname},
			unpack lines
		}}}
		
		-- print "rangelines"
		-- for tline in *rangelines
			-- print tline
		{newline,rangelines}
