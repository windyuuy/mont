...
build_grammar = wrap_env debug_grammar, (root) ->
	...
	g = P {
		...
		-- CheckParam: C"(" * Ct(Exp * (sym"," * Exp)^0)^-1 * C")"
		ExpListTable: Ct(Exp) * (sym"," * Ct(Exp))^0
		RangeParamLit: sym"(" * Ct(
				TableValueList^-1 * sym","^-1 *
				(SpaceBreak * TableLitLine * (sym","^-1 * SpaceBreak * TableLitLine)^0 * sym","^-1)^-1
			) * White * sym")" / mark"table"
		-- CheckParam: Ct(C"(" * Ct(ExpListTable^0)^-1 * C")") + TableLit
		CheckParam: RangeParamLit
		TypeName: ( ((#P("'") + #P('"'))*String) + (Cc("'")*Name)/mark"string" ) * CheckParam^-1
		NameTyped: Name * TypeCatch^-1
		...
		-- TableBriefIter: C"[" * C(P"." + P"")^0 * C(P"^*" + P"*^" + TableBriefIter) * C(P"." + P"")^0 * C"]"
		-- OpForeachTable: Value * Ct(TableBriefIter) * Value

		ActiveAgWithOrder:(
			Cc"default"		* P"$" * (Num/mark"order")^-1 * P"="	* (Value/mark"default") +
			Cc"set"			* P"$" * (Num/mark"order")^-1 * P":"	* (Value/mark"set_obj") +
			Cc"ins_default" * P"$" * (Num/mark"order")^-1
			)
		ActiveAg: Ct(
			ActiveAgWithOrder +
			Cc"add_set"		 										* (Value/mark"set_obj") +
			Cc"skip_default"
			)
		ActiveBriefAg: P","
		...
		FuncOp: S"^"
		SubValue: Value
		SubValueList: SubValue * (C(FuncOp) * SubValue)^0
		FlowProcMap: (
				Ct(sym"<~" * SubValueList * sym">")/mark"right" +
				Ct(sym"<" * SubValueList * sym"~>")/mark"left"
			)/build_flowprocmap *
			Ct(InvokeArgs^-1) / join_chain
		
		SwitchTable: Ct(sym'[' * Value * sym']' * (FnArgs + Cc{}) * Value) / organize_switch_table
		...
		Callable: pos((NameTyped) / mark"ref") + SelfName + VarArg + Parens / mark"parens"
		...
		FnArgDef: Ct((NameTyped + SelfName) * (sym"=" * Exp)^-1)
		...
		CloseFunLit: sym"^"^-1 * sym"{" * White * CloseFnArgsDef *
			(Cc"slim") *
			(Body + Ct"") * White * sym"}" / mark"fndef"


		NameList: NameTyped * (sym"," * NameTyped)^0
		...
		ExpListTyped: ExpTyped * (sym"," * ExpTyped)^0
		ExpListLowTyped: ExpTyped * ((sym"," + sym";") * ExpTyped)^0
